class ScrollerVis {
  ...
  initVis() {
    ...
    vis.dataLongestAssignments = [...vis.data].sort((a,b) => b.time - a.time)[0];
    ...
    vis.rect = vis.chart.selectAll('rect')
        .data(vis.data, d => d.name).join('rect'); //vis.data instead of just data
  ...
  step3() {
    ...
    vis.rect.transition()
        .attr('opacity', 1)
        //Original --> .attr('fill', d => d.trail==vis.dataLongestHike.trail ? vis.colorScale('highlighted') : vis.colorScale('inactive'))
        .attr('fill', d => d.name ==vis.dataLongestAssignments.name ? vis.colorScale('highlighted') : vis.colorScale('inactive'))        
        .attr('width', d => vis.config.cellWidth)
        .attr('height', d => vis.config.cellHeight)
        .attr('x', (d, i) => i % vis.config.columns * (vis.config.cellWidth + vis.config.cellSpacing))
        .attr('y', (d, i) => Math.floor(i / vis.config.columns) % vis.config.rows * (vis.config.cellHeight + vis.config.cellSpacing));
